...
import { DaySchedule, ScheduleItem, Project, Lesson } from '../../types';
...
const ScheduleView: React.FC<ScheduleViewProps> = ({ schedule, projects, lessons = [], onAddItem, isEditor = false }) => {
    ...
    <div className="space-y-10 relative">
              ...
              <div className="p-8 space-y-6">
                {filteredItems.length > 0 ? (
                    ...
                    return (
                      <div key={idx} className={`p-8 rounded-[2rem] border flex flex-col gap-4 relative hover:scale-[1.01] transition-transform ${typeStyle} shadow-sm`}>
                        ...
                        <div className="absolute right-8 bottom-8 flex gap-3">
                          <button onClick={() => setSelectedPreview(item)} className="px-8 py-3 rounded-2xl bg-white text-slate-800 font-black text-[11px] shadow-sm hover:shadow-xl transition-all border border-slate-100 uppercase tracking-widest">Preview</button>
                        </div>
                      </div>
                    );
                  })
                  ...
                  <div className="flex flex-col items-center justify-center py-16 text-slate-300 border-2 border-dashed border-slate-50 rounded-[2rem]">
                    ...
                    <p className="font-black text-xs uppercase tracking-widest text-slate-400">No Activities Scheduled</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {isEditor && (
        <div className="fixed bottom-24 right-6 z-50">
          <button onClick={() => setIsAddingSchedule(true)} className="w-14 h-14 rounded-full flex items-center justify-center text-white text-xl shadow-2xl transition-all duration-300 active:scale-95 bg-gradient-to-tr from-purple-600 to-indigo-600 hover:rotate-90"><i className="fa-solid fa-plus"></i></button>
        </div>
      )}
      ...
      {selectedPreview && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-4xl rounded-[3rem] overflow-hidden shadow-2xl flex flex-col md:flex-row relative animate-in zoom-in-95 duration-300 border border-white/20">
            <button onClick={() => setSelectedPreview(null)} className="absolute top-6 right-6 z-10 w-12 h-12 rounded-full bg-black/20 hover:bg-black/40 backdrop-blur-md text-white flex items-center justify-center transition-all shadow-xl"><i className="fa-solid fa-times"></i></button>
            <div className="md:w-1/2 h-64 md:h-auto relative">
              <img src={selectedPreview.imageUrl || 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&q=80&w=1200'} className="w-full h-full object-cover" alt={selectedPreview.title} />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
              <div className="absolute bottom-10 left-10 right-10">
                <div className="px-4 py-1.5 bg-purple-600 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] mb-4 inline-block shadow-lg">{selectedPreview.type}</div>
            ...
            <div className="md:w-1/2 p-10 md:p-14 overflow-y-auto max-h-[70vh] md:max-h-none flex flex-col bg-white">
              <div className="grid grid-cols-2 gap-4 mb-10">
                <div className="space-y-1">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-sm text-slate-700"><i className="fa-regular fa-clock mr-2 text-purple-500"></i>{selectedPreview.time}</div>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Target Audience</p>
                  <div className="bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-bold text-sm text-slate-700"><i className="fa-solid fa-users mr-2 text-purple-500"></i>{selectedPreview.audience}</div>
                </div>
              </div>
              <p className="text-slate-600 leading-relaxed font-medium">{selectedPreview.description}</p>
              <div className="mt-auto pt-12 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-400"><i className="fa-solid fa-chalkboard-user"></i></div>
                  <div>
                ...
                <button className="px-10 py-4 bg-purple-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-purple-100 hover:bg-purple-700 transition-all active:scale-95">Book This Slot</button>
              </div>
            </div>
          </div>
        </div>
